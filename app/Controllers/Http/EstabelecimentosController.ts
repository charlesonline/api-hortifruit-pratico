...
export default class EstabelecimentosController {
    public async pedidos({ response, auth }: HttpContextContract) {
        ...
        const pedidos = await Pedido.query().where("estabelecimento_id", estabelecimento.id).preload("cliente").preload("pedido_status", (statusQuery) => {
    ...
    public async show ({ params, response }: HttpContextContract) {
        ...
        }).preload("meiospagamento").firstOrFail();
